...
def query():
    ...
    cur.execute("WITH g1 AS (SELECT cust, COUNT(quant) AS count_1_quant FROM sales WHERE state = 'NY' GROUP BY cust), g2 AS (SELECT cust, SUM(quant) AS sum_2_quant FROM sales WHERE state = 'NJ' GROUP BY cust), g3 AS (SELECT cust, MAX(quant) AS max_3_quant FROM sales WHERE state = 'CT' GROUP BY cust) SELECT DISTINCT sales.cust, COALESCE(g1.count_1_quant, 0) AS count_1_quant, COALESCE(g2.sum_2_quant, 0) AS sum_2_quant, COALESCE(g3.max_3_quant, 0) AS max_3_quant FROM sales LEFT JOIN g1 ON sales.cust = g1.cust LEFT JOIN g2 ON sales.cust = g2.cust LEFT JOIN g3 ON sales.cust = g3.cust")
